...
exports.installScript = function (name, script, options, root) {
    ...
    var packagePath = Path.join(exports.findProjectRoot(root), 'package.json');
    var content = Fs.readFileSync(packagePath, { encoding: 'utf8' };
    ...
    var lastCurly = content.lastIndexOf('}');
    var head = content.substring(0, pos);
    var tail = content.substring(lastCurly + 1);
    ...
    if (!package.hasOwnProperty('scripts') ||
        ...
        Fs.writeFileSync(packagePath, head + JSON.stringify(package, null, 2)} + tail, { encoding: 'utf8' });
    }
};
