...
exports.copy = function (source, target, options) {
    ...
    console.log(projectRoot, targetPath);

    if (!(new RegExp('^' + projectRoot.replace('/', '\/').replace('\\', '\\\\') + Path.sep + '\.*$')).test(targetPath)) {
...
exports.installHooks = function (hooks) {

    if (!Array.isArray(hooks)) {
        hooks = [hooks];
    }

    var gitRoot = exports.findGitRoot();
